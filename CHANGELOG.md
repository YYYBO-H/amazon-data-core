...
- 默认通过临时的 localhost 页面安全录入 Amazon 凭证，使没有交互式 TTY
  的本机 Agent 也能完成安装流程；
- 授权与首次同步在独立后台任务中运行，不会随 Agent 命令会话结束而退出；
- 兼容 Chrome 翻译或隐私扩展产生的 opaque origin，同时继续拒绝外部来源提交；
- 保留真实终端录入作为备用方式，并增加跨来源提交、密钥回显和文件权限测试。
